...
class chanVese (object):
    ...
    def build_model(self):
        ...
        self.encoder    = EncoderSimple(in_channel=3)
        self.maskDecoder    = DecoderSimple()
        self.encoder = self.encoder.to(self.device)
        self.maskDecoder = self.maskDecoder.to(self.device)

        # init_weights(self.maskDecoder, 'kaiming')
        # init_weights(self.foreDecoder, 'kaiming')
        # # init_weights(self.backDecoder, 'kaiming')
        # init_weights(self.encoder, 'kaiming')
    ...
    def run(self, epoch, data_loader, work):
        ...
        for batch_num, (input, labelTarget) in enumerate(data_loader):
            ...
            if work == 'train':
                ...
                # foreCenter, backCenter = self.lapalce(input, th_mask, num_iter=1)

                # total variation for smooth, L1 loss for area of region
                maskSmooth = self.loss.tv(mask)

                # foreSegLoss, backSegLoss = self.loss.segmentSmoothLoss(input, mask, foreCenter, backCenter)
                foreSegLoss, backSegLoss = self.loss.segmentConstantLoss(input, mask)

                # foreSmooth = self.loss.tv(foreCenter)
                # backSmooth = self.loss.tv(backCenter)

                mumfordLoss = foreSegLoss + backSegLoss
                
                # regularization = self.ms * maskSmooth + self.mt * (foreSmooth + backSmooth)
            ...
            # foreRegular += (foreSmooth.item() * input.size(0))
            # backRegular += (backSmooth.item() * input.size(0))
    ...
    def runner(self):
        ...
        for epoch in range(1, self.nEpochs + 1):
            ...
            # if epoch == self.nEpochs:
                # self.plot.plotResult(epoch, self.train_loss, self.val_loss)
                # saveModel(self.encoder, self.maskDecoder, self.config, 'chanVese')
